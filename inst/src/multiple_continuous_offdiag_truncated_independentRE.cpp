...
Type objective_function<Type>::operator() (){
  ...
  for (int j = 0; j < wh; j++){
    ...
    int t = tem.size();
      for (int i = 0; i < (t-1); i++){
	if(covs(i) == 0){
	  q(0) = exp(log_baseline(0) + log_b1_12 + u(j,0));
	  q(1) = exp(log_baseline(1) + log_b1_21 + u(j,1));
	}else{
	  if(covs(i) >= truncation){
	    q(0) = exp(log_baseline(0) + u(j,0));
	    q(1) = exp(log_baseline(1) + u(j,1));
	  }else{
	    q(0) = exp(log_baseline(0) + log_b2_12*covs(i) + u(j,0));
	    q(1) = exp(log_baseline(1) + log_b2_21*covs(i) + u(j,1));
	  }
	}
